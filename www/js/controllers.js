angular.module('fans.controllers', [])
.controller('DashCtrl', ['$scope','$ionicSideMenuDelegate', function($scope,$ionicSideMenuDelegate){
}])
  .controller('BendiCtrl', ['$scope', function($scope){
  }])
  .controller('PersonCtrl', ['$scope', function($scope){
  }])
  .controller('QunCtrl', ['$scope', function($scope){
  }])
  .controller('GzhCtrl', ['$scope', function($scope){
  }])
 ...
 // }])
  .controller('UinfoCtrl', ['$scope','$state','UserSer','$cordovaToast',function($scope,$state,UserSer,$cordovaToast){
  ...
  .controller('MyuanCtrl', ['$scope','$state','UserSer','$cordovaToast',function($scope,$state,UserSer,$cordovaToast){
    ...
    $scope.huan=function (hid) {
      ...
      UserSer.Hyuan({hid:hid}).$promise.then(function (succ) {
        ...
        if (succ.code==1) {
          $scope.yuans.splice($scope.yids.indexOf(hid), 1);
        };
      }, function (error) {
        $cordovaToast.showShortCenter('网络错误!');
      })
  ...
  .controller('zhuqianCtrl', ['$scope','$stateParams','$state','ToolSer','$cordovaToast','ShareSer', function($scope,$stateParams,$state,ToolSer,$cordovaToast,ShareSer){
    ...
    $scope.shareQian=function () {
      $scope.url='http://www.zhongfox.com/api.php?s=/Pcweb/shareQian/id/'+$scope.qdid;
  ...
  .controller('ModalCtrl', ['$scope','$ionicActionSheet','$ionicPopup','ToolSer','$state','$cordovaToast',function($scope, $ionicActionSheet, $ionicPopup,ToolSer,$state,$cordovaToast) {
